...
public class OTSByDateTest implements IRetryAnalyzer, IAnnotationTransformer {
	...
	public void headerNameAndClientIdOTSbyDate() {
		...
		String clientId = webAppHelper.findElementsByXPath("//a[contains(@class,'c-subheader-nav-link')]").get(1)
				.getText();
	...
	public void footerLeftSideText() {
		...
		String versionName = webAppHelper.findElementByClassName("mfs-start").getText();

		logger.info("clicking and Asserting version text");
		Assert.assertEquals(versionName, "v2022.10.13");
		webAppHelper.findElementByClassName("mfs-start").click();
